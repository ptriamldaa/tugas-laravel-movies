...
<section class="container my-24 mx-auto">
    <form action="{{ route('movies.update', $movie->id) }}" method="POST" enctype="multipart/form-data"
        ...
        <!-- Form Title -->
        <h1 class="text-3xl font-bold text-center text-gray-800 dark:text-gray-100 mb-10">Edit Data Film</h1>
        ...
        <!-- Year -->
        <div class="relative z-0 w-full mb-5 group">
            <input type="number" name="year" id="year" value="{{ old('year', $movie->year) }}" min="1900" max="2099"
                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" " required />
            <label for="year"
            ...
            <div class="flex items-center space-x-4 mt-2">
                <label for="yes" class="flex items-center">
                    <input type="radio" name="available" id="yes" value="1" class="text-blue-600"
                        {{ old('available', $movie->available) == 1 ? 'checked' : '' }}>
                ...
                <label for="no" class="flex items-center">
                    <input type="radio" name="available" id="no" value="0" class="text-blue-600"
                        {{ old('available', $movie->available) == 0 ? 'checked' : '' }}>
